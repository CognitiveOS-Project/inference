...
```bash
# Standard build (detects llama.cpp, falls back to mock)
make build
...
```bash
make build    # Builds both coginfer and cograw
make test     # Run tests
make lint     # Run go vet
make clean    # Remove build artifacts
```

Requires CGo and vendored llama.cpp for production builds. See `scripts/build.sh` for the self-contained bootstrap.
...
```bash
make build    # Compile to build/bin/cognitiveos-inference and build/bin/cograw
make test     # Run tests
make lint     # Run go vet
make clean    # Remove build artifacts
```
